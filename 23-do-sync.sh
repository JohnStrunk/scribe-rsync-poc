...
kubectl --kubeconfig "${DEST_KUBECONFIG}" -n wp delete volumesnapshot/dest-data-wp-mariadb-0 volumesnapshot/dest-wp-wordpress
...
echo ==== wordpress sync log ====
kubectl --kubeconfig "${SRC_KUBECONFIG}" -n wp logs job/scribe-rsync-source-wp | tail -n 25
echo ==== mariadb sync log ====
kubectl --kubeconfig "${SRC_KUBECONFIG}" -n wp logs job/scribe-rsync-source-maria | tail -n 25
